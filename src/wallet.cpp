...
// stronghands: optional setting to unlock wallet for block minting only;
...
// stronghands: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // stronghands: timestamp must not exceed spend time
...
bool CWallet::CreateTransaction(const vector<pair<CScript, int64> >& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, int64& nFeeRet, const CCoinControl* coinControl)
{
...

    {
        ...
        */
                ...
                // stronghands: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    else
                    {
                        ...
                        if (!GetBoolArg("-avatar")) // stronghands: not avatar mode
                        {
...
// stronghands: create coin stake transaction
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64 nSearchInterval, CTransaction& txNew)
{
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
            ...
            && pcoin.first->GetHash() != txNew.vin[0].prevout.hash)
        {
            ...
            if (pcoin.first->nTime + STAKE_MAX_AGE_2 > txNew.nTime)
                continue;
            txNew.vin.push_back(CTxIn(pcoin.first->GetHash(), pcoin.second));
            nCredit += pcoin.first->vout[pcoin.second].nValue;
            vwtxPrev.push_back(pcoin.first);
...
// stronghands: check 'spent' consistency between wallet and txindex
// stronghands: fix wallet spent state according to txindex
...
// stronghands: disable transaction (only for coinstake)
